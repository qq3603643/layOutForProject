	...
	(() =>
	{
		var $ipt_vcode = $('#J_vcode'),
			...
			Main =
			{
				...
				initCount()
				{
					let ensuremobile_vCode_starttime = Ls.get(`ensuremobile_vCode_starttime_${ $.trim($('#J_orginalmobile').text()) }`);
				...
				getvCode()
				{
					...
					$.ajax(
					{
						url: '/account/sendSms',
						type: 'POST',
						data: { mobile: $.trim($('#J_orginalmobile').text()), type: 'VALIDATE_CURRENT_MOBILE' },
						beforeSend()
						{
							...
							$this.addClass('count')
					      	...
					      	//init count
					      	Ls.set(`ensuremobile_vCode_starttime_${ $.trim($('#J_orginalmobile').text()) }`, new Date() * 1);
					      	$btn_getvcode.addClass('count')
					      			     .html($CONFIG_G.btn_getvcode[1]);
					...
					.done((da) =>
					{
						if(da.status != 1)
						{
							...
							Ls.set(`ensuremobile_vCode_starttime_${ $.trim($('#J_orginalmobile').text()) }`, new Date() * 1 - $CONFIG_G.difftime_getvcode * 1e3);
							return;
						}
					})
					.fail(() =>
				...
				trackerConfirmMobile(resultcode)
				{
					if(!resultcode)
						return;

					Tracker.trace('click', 'clickconfirmmobile',
					{
