/*===================================
 * @description: 轮播图(dcec_carousel.js)
 * @author: 马腾
 * @update: 马腾 (2013-09-02)
 **********使用说明********************
调用：
$("#focus").dcecCarousel({
	effects : 'fade',
...
(function($){
	...
	$.fn.dcecCarousel = function(options){
		...
		var length = slider.find(slider.options.liBox).length;
		methods = {};
		...
		methods = {
			init : function(){
				...
				this.ulBox = slider.options.ulBox;
				this.liBox = slider.options.liBox;
			...
			initEffects : function(){
				...
				switch(this.effects){
					case 'fade' :
						...
						slider.find(this.liBox).css({'position':'absolute','left':0,'top':0,'display':'none'}).eq(0).css({'display':'block'}).addClass(this.hasSelected);
						
					break;
					
					case 'horizontal' :
						var _sliderUl = slider.find(this.ulBox);
						var	_sliderLi = _sliderUl.find(this.liBox);
			...
			fade : function(index){
				this.classNav(index);
				var _sliderList = slider.find(this.liBox);
				_sliderList.stop().fadeOut(this.speed).removeClass(this.hasSelected).eq(index).stop(true,true).fadeIn(this.speed).addClass(this.hasSelected);
			},
			...
			loop: function(index,rotation){
				...
				if(rotation=="left"){
					...
					_sliderUl.find(_this.liBox+":last").prependTo(_sliderUl);
					...
					_sliderUl.stop().animate({'marginLeft':-_sliderListW},_this.speed,function(){
						...
						_sliderUl.find(_this.liBox+":first").appendTo(_sliderUl);
					});
				}
			},
